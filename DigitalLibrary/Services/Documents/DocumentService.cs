    ...
    public class DocumentService : IDocumentService
    {
        ...
        public async Task<List<Author>> GetAuthors()
        {
            ...
            .Where(a =>
                a.Documents.Any(d =>
                    !d.IsDeleted &&
                    d.Submissions.Any(s => s.Status == "Accept")
                )
            )
            .ToListAsync();
        }
        ...
        public async Task<object> AdminGetAllAsync(string? authorId, string? collectionId, string? communityId, string? type, string? keyword, string sortBy, int page, int pageSize)
        {
            var query = _context.Documents
            .Where(d =>
            !d.IsDeleted
        ...
        public async Task<List<DocumentListDto>> SearchAsync(string keyword)
        {
            ...
            var list = await _context.Documents
                .Where(d => !d.IsDeleted && d.Submissions.Any(s => s.Status == "Accept") &&
                    (
        ...
        public async Task<List<DocumentList2Dto>> GetByViewsAsync()
        {
            var result = await _context.ReadingDocuments
                .Where(r => r.IsCounted)
                .Where(d => _context.Submissions.Any(s => s.DocumentId == d.DocumentID && s.Status.Equals("Accept")))
