...
namespace DigitalLibrary.Services.Submissions
{
    public class SubmissionService : ISubmissionService
    {
        ...
        public async Task<string> PrereviewAsync(Guid submissionId, string reviewerId)
        {
            ...
            var lastReviewByReviewer = await _context.SubmissionHistories
                .Where(h =>
                    h.SubmissionId == submissionId &&
                    h.Action == "Review" &&
                    h.PerformedBy == reviewerId
                )
                .OrderByDescending(h => h.CreatedAt)
                .FirstOrDefaultAsync();

            var lastReviseByAuthor = await _context.SubmissionHistories
                .Where(h =>
                    h.SubmissionId == submissionId &&
                    h.Action == "Revise"
                )
            ...
            {
                if (lastReviseByAuthor == null ||
                    lastReviseByAuthor.CreatedAt <= lastReviewByReviewer.CreatedAt)
                {
